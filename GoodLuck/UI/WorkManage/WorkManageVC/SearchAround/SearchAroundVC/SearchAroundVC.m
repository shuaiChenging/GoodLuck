...
@interface SearchAroundVC ()<MAMapViewDelegate,PlaceAroundTableViewDeleagate>
...
- (void)initTableview
{
    self.tableview = [PlaceAroundTableView new];
...
- (void)sure
{
    if ([Tools isEmpty:self.amapPOI.name])
    {
        return;
    }
    
    [self.subject sendNext:self.amapPOI];
