...
- (void)createCameraView
{
    ...
    CGRect smallFrame = CGRectMake((kScreenWidth - 300)/2, 160, 300, 300);
...
//    //拍照视图 上拉按钮 拍照按钮
//    UIButton *upBtn = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth/2-60, kScreenWidth/16-s1, 120, 20)];
//    upBtn.tag = 1001;
...
//
//    UIButton *photoBtn = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth/2-30,kScreenWidth/16-s1,60, 60)];
//    photoBtn.tag = 1000;
////    photoBtn.hidden = YES;
...
//    [self.view bringSubviewToFront:photoBtn];
    ...
    UILabel *label = [UILabel labelWithText:@"请把车牌号放入识别框中"
    ...
    UILabel *onlineLb = [UILabel labelWithText:@"在线识别"
                                          font:[UIFont systemFontOfSize:font_14]
                                     textColor:[UIColor whiteColor]
                                     alignment:NSTextAlignmentCenter];
    onlineLb.backgroundColor = [UIColor jk_colorWithHexString:COLOR_BLUE];
    onlineLb.layer.masksToBounds = YES;
    onlineLb.layer.cornerRadius = 45;
    onlineLb.userInteractionEnabled = YES;
    [onlineLb jk_addTapActionWithBlock:^(UIGestureRecognizer *gestureRecoginzer) {
        [weakself photoBtn];
    }];
    [self.view addSubview:onlineLb];
...
-(void)sliderChange:(UISlider*)sender {
    _preview.affineTransform = CGAffineTransformMakeScale(sender.value, sender.value);
}
    ...
    [_captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         if (imageSampleBuffer != NULL) {
             ...
             [[AipOcrService shardService] detectPlateNumberFromImage:temp_image withOptions:nil successHandler:^(id result) {
                 if (result[@"words_result"])
                 {
                     if ([result[@"words_result"] isKindOfClass:[NSDictionary class]])
                     {
                         NSString *carNumber = result[@"words_result"][@"number"];
                         
                         NSMutableDictionary* resultDict = [NSMutableDictionary new];
                         resultDict[@"image"] = temp_image;
                         resultDict[@"text"] = carNumber;
                         [weakself performSelectorOnMainThread:@selector(readyToGetImage:) withObject:resultDict waitUntilDone:NO];
                     }
                 }
                 else
                 {
